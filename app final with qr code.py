...
class Login(QMainWindow):
    def __init__(self):
      ...
      #  layout.addWidget(username)
        ...
        password.setStyleSheet(" QLineEdit { QLineEdit-password-character: 9679; line-height: 42px; background-color: #FFFFFF; font-weight: 400; font-size: 20px; font-family: Roboto; font-style: normal; color: #979797; border-radius: 20px; position: absolute; box-shadow:50x; padding-left: 24px;}" )
...
class NewWindow5(QMainWindow):
    ...
    def review_transaction(self):
        ...
        #qrcode_img = Q
        qrcode_generated = requests.get('https://api.github.com/users') 
        print(qrcode_generated.content)
        qr = qrcode.QRCode(version=1,box_size=10,border=5)
        qr.add_data(qrcode_generated)
        qr.make()
        img = qr.make_image()
       # pos = (img.size[1], img.size[1])
